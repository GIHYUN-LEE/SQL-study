...
**2. 발급회원번호 format** <br>
발급회원번호의 값이 `SYN_0`형태의 varchar타입으로 제공되어 있어 INT 형식으로 포멧
...
<details>
<summary>1. LIST 파티셔닝</summary><br>	
<details>
...
<details>
<summary>2. RANGE 파티셔닝</summary><br>	
<details>
...
# ⌛ 실행 코드
`PROCEDURE`를 이용하여 파티셔닝 테이블 별 500번씩 `연체연도 = 2000 and 발급회원번호=9945` 조건을 조회합니다. <br>
### 📌 PROCEDURE 란
...
- `KEY`는 **MySQL 내부의 비공개 해시 알고리즘을 사용**하여, 분산 방식의 **예측이 불가** 하며 데이터가 **편중**될 수 있다.
...
📢 **결론** <br>
MYSQL 기준 **2단계 파티셔닝**까지 지원하며, **메인 파티셔닝은 RANGE와 LIST 서브 파티셔닝은 HASH와 KEY** 사용 가능
